...
#undef _PADr_KAZE
    ...
    public:
        ...
        bool insert(u32 item_index)
        {
            ...
            return emplace(item_index, key, slot, newly_added);
        }

        Value* find(const Key& key)
        {
            u32 const slot = m_hash_policy.index_for_hash(hash_object(key), m_num_slots_minus_one);
            entry_t*  it   = m_meta->get_item(slot);
            for (s8 distance = 0; it->distance_from_desired() >= distance; ++distance, ++it)
            {
                if (compares_equal(key, *m_meta->get_item(it->value())))
        ...
        u32 max_size() const { return m_meta->get_cap_max(); }
        ...
        {
            entry_t* current_entry         = m_meta->get_item(slot);
        ...
        entry_t* emplace_new_key(s8 distance_from_desired, entry_t* current_entry, u32 item_index, Key* item_key, u32 slot, bool& out_newlyadded)
        {
        ...
        void grow()
        {
            ...
            do
            {
                ...
                m_meta->set_capacity(new_max_slots + new_max_lookups);
        ...
        bool rehash(u32 new_num_slots)
        {
            ...
            // in-place rehashing
            entry_t* it     = m_meta->get_item(0);
            entry_t* it_end = m_meta->get_item(max_slots());
            while (it < it_end)
            {
                if (!it->is_empty())
                    it->set_old();
            ...
            entry_t* special_end_item = m_meta->get_item(new_num_slots + new_max_lookups - 1);
            while (it <= special_end_item)
            {
                it->set_empty();
                ++it;
            }
            special_end_item->distance_from_desired(entry_t::cspecial_end_value);
            ...
            for (u32 i = 0; i < end; i++)
            {
                ...
                if (it->is_old() && it->has_value())
                {
                    ...
                    do
                    {
                        ...
                        entry_t*  prev        = displace(it->value(), slot, should_grow);
                        if (should_grow)
                        {
        ...
        entry_t* displace_new_key(s8 distance_from_desired, entry_t* current_entry, u32 item_index, bool& grow)
        {
            ...
            for (++distance_from_desired, ++current_entry;; ++current_entry)
            {
                if (current_entry->is_empty())
                {
                    ...
                    ++m_num_elements;
                    return m_meta->get_item(prev_item_index);
                }
