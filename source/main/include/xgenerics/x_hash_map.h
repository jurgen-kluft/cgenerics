...
#undef _PADr_KAZE
    ...
    template <typename Key, typename Value, typename HashPolicy = fibonacci_hash_policy, typename EntryHT = s8, typename EntryLT = u16> class hashmap_t
    {
        ...
        {
            // reset to an empty state just means that we have to reset the entry_ht and the entry_lt array's as well
            // as resetting some member variables.
            // what to do with the keys and values ? what if we do not own them and they could already have contained some items
            m_num_elements        = 0;
            m_num_slots_minus_one = 0;
            m_max_lookups         = cmin_lookups - 1;
        }
    ...
    public:
        ...
        inline Value& operator[](const Key& key)
        {
            ...
            return *m_values->get_item(entry.value());
        }
        ...
        entry_type emplace(s32 item_index, Key const* item_key, u32 slot, bool& out_newlyadded)
        {
        ...
        entry_type emplace_new_key(s8 distance_from_desired, entry_type current_entry, u32 item_index, Key const* item_key, u32 slot, bool& out_newlyadded)
        {
        ...
        bool rehash(u32 new_num_slots)
        {
            ...
            while (it < it_end)
            {
                // if (!it.is_empty()) // not really needing this branch, do we?
                it.set_old();
                it.next();
            }

            // the newly allocated slots, set them slots to empty
            ...
            for (u32 i = 0; i < end; i++)
            {
                ...
                if (it.is_old() && it.has_value())
                {
                    ...
                    do
                    {
                        Key const* key         = m_keys->get_item(it.value());
                        u32 const  slot        = compute_slot_for_key(*key);
                        ...
                        if (should_grow)
                        {
                            ...
                            // We still have a key slot that reaches 'max lookups', better
